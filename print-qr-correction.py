...
# for fold, (fold_train_idx, fold_eval_idx) in enumerate(KFold(len(train_y), n_splits=n_folds, shuffle=True, random_state=2016)):
...
for fold, (fold_train_idx, fold_eval_idx) in enumerate(
        KFold(n_splits=n_folds, shuffle=True, random_state=2016).split(train_x, y=train_y)):
    fold_train_x = train_x[fold_train_idx]
    fold_train_y = train_y[fold_train_idx]
